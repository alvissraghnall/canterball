<script lang="ts">
  ...
  import { FIELD, PIECE, SHOT, GOALIE_BOX } from '@canterball/shared';
  import { CANVAS, fieldToCanvas, canvasToField } from '$lib/game/constants';

  export let state: GameState | null = null;
  export let mySide: Team | null = null;
  export let isMyTurn: boolean = false;
  export let shotPath: Point[] = [];
  export let shotAnimating: boolean = false;
  export let goalieWindowActive: boolean = false;
  export let onMovePiece: (pieceId: string, x: number, y: number) => void = () => {};
  export let onDeclareShot: (pieceId: string, x: number, y: number, power: number) => void = () => {};
  export let onRepositionGoalie: (x: number, y: number) => void = () => {};
  ...
  function drawPieces() {
    ...
    for (const piece of state.pieces) {
      ...
      const radius = piece.type === 'GOALIE' ? PIECE.GOALIE_RADIUS * 4 : PIECE.PLAYER_RADIUS * 4;
  ...
  function drawBall() {
    if (!state) return;
    const { x, y } = fieldToCanvas(state.ball.x, state.ball.y);
  ...
  function handleCanvasClick(e: MouseEvent) {
    if (!state || !mySide || !isMyTurn) return;
  ...
  function handleCanvasRightClick(e: MouseEvent) {
    e.preventDefault();
    if (!state || !mySide || !isMyTurn || !selectedPiece) return;
    if (shotAnimating) return;
    ...
    <div class="actions">
      {#if shotTarget}
        <div class="power-slider">
          ...
          <input id="shot-power" type="range" min={SHOT.MIN_POWER} max={SHOT.MAX_POWER} bind:value={shotPower} />
        </div>
        <button onclick={() => { const sel = selectedPiece; const st = shotTarget; if (sel && st) { onDeclareShot(sel, st.x, st.y, shotPower); selectedPiece = null; shotTarget = null; } }}>
          Shoot!
        </button>
      {:else if moveTarget}
