...
export const isMyTurn = derived([gameState, playerSide], ([$gameState, $playerSide]) => {
	if (!$gameState || !$playerSide) return false;
	return $gameState.currentTurn === $playerSide;
});

export const myTeamPieces = derived([gameState, playerSide], ([$gameState, $playerSide]) => {
	if (!$gameState || !$playerSide) return [];
	return $gameState.pieces.filter((p) => p.team === $playerSide);
});

export const opponentPieces = derived([gameState, playerSide], ([$gameState, $playerSide]) => {
	if (!$gameState || !$playerSide) return [];
